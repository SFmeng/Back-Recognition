...
class FewShotClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = backbone if backbone is not None else nn.Identity() # 主干网络
        self.use_softmax = use_softmax # 是否使用softmax
    ...
    def process_support_set(
        ...
        """
        self.compute_prototypes_and_store_support_set(support_images, support_labels) # 计算支撑集的原型表示
    ...
    def compute_prototypes_and_store_support_set(
        ...
        self.support_features = self.compute_features(support_images) # 提取支撑集特征
        self._raise_error_if_features_are_multi_dimensional(self.support_features)  # 检查支撑集维度
        self.prototypes = compute_prototypes(self.support_features, support_labels) # 保存在支持集原型表示变量里面

    @staticmethod
    def _raise_error_if_features_are_multi_dimensional(features: Tensor): # 检查特征是否是多维的
