...
class PrototypicalNetworks(FewShotClassifier): # 继承自小样本分类器
    """
    ...
    def forward(
        ...
        query_features = self.compute_features(query_images) #计算查询集特征
        self._raise_error_if_features_are_multi_dimensional(query_features) #检查特征是否是多维的

        # Compute the euclidean distance from queries to prototypes
        scores = self.l2_distance_to_prototypes(query_features) #

        return self.softmax_if_specified(scores) # 返回分类分数
